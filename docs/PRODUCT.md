...
DoneWitness verifies the resulting worktree, not the identity or transcript of the builder. A human, Codex, Claude Code, Cursor, or another tool can produce the code under test.
...
3. The user invokes DoneWitness with the plan and application start command.
4. DoneWitness validates the inputs, starts or connects to the local application, and runs declared checks.
5. Each criterion receives `PASS`, `FAIL`, or `UNKNOWN` plus evidence references.
6. DoneWitness aggregates the run and writes a proof receipt.
7. The user reviews the receipt and artifacts, then independently decides whether to accept the implementation.

## Non-goals

For v0.1, DoneWitness will not provide:
...
- complete the workflow without an LLM API, hosted DoneWitness service, or builder-specific integration.

Success is not measured by how confidently DoneWitness describes code. It is measured by whether another person can inspect what ran, what was observed, and why the verdict follows.
